'''
NOTE: when we don't need to create third variable, it make the changes in original variable set.
AND when we create third variable, it doesnot modify the original set items.
'''
#All Methods of Sets:
# add()
# update()
...
# How to add attach two sets using update()
name1 = {"omkar", "kunal"}
name2 = {"Shubh", "Raj"}
name1.update(name2) # no need to create the 3rd variable
print(name1)

# How to join the 2 set using union() method.
set1 = {'a', 'b', 'c', 'd'}
set2 = {1, 2, 3, 4, 5}
...
# Remove the last items using the pop(), method will gives you wrong output.
color = {"white", "black", "yellow", "green", "blue"}
color.pop()
print(color)
...
# Del keyword is used to delete the set completly
color = {"white", "black", "yellow", "green", "blue"}
del color
# print(color)
...
# Print only duplicate items from both sets using intersection_update() method
...
# Print only duplicate items from sets using intersection() method but need to create new variable.
...
# IMP: By default set cannot add the duplicate items, but we have method as well,
...
# symmetric_difference() method wil return a new set that contain only the elements that contain only the elements that are NOT present in Both sets. --> same as symmetric_difference_update().
